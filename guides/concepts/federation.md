---
title: Federation
replacement: https://developers.aiblocks.io/docs/glossary/federation/
---

The AiBlocks federation protocol maps AiBlocks addresses to more information about a given user. It's a way for AiBlocks client software
to resolve email-like addresses such as `name*yourdomain.com` into account IDs like: `GCCVPYFOHY7ZB7557JKENAX62LUAPLMGIWNZJAFV2MITK6T32V37KEJU`. AiBlocks addresses provide
...
AiBlocks addresses are divided into two parts separated by `*`, the username and the domain.

For example:  `jed*aiblocks.io`:
* `jed` is the username,
* `aiblocks.io` is the domain.
...
Note that the `@` symbol is allowed in the username. This allows for using email addresses in the username of an address. For example: `maria@gmail.com*aiblocks.io`.

## Supporting Federation

### Step 1: Create a [aiblocks.toml](./aiblocks-toml.md) file

Create a file called aiblocks.toml and put it at `https://YOUR_DOMAIN/.well-known/aiblocks.toml`.

### Step 2: Add federation_url

Add a `FEDERATION_SERVER` section to your aiblocks.toml file that tells other people the URL of your federation endpoint.
...
The federation URL specified in your aiblocks.toml file should accept an HTTP GET request and issue responses of the form detailed below.

Instead of building your own server you can use the [`federation server`](https://github.com/aiblocks/go/tree/master/services/federation) built by AiBlocks Development Foundation.

## Federation Requests
You can use the federation endpoint to look up an account id if you have a aiblocks address. You can also do reverse federation and look up a aiblocks addresses from account ids or transaction ids. This is useful to see who has sent you a payment.
