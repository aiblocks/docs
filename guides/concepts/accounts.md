---
title: Accounts
replacement: https://developers.aiblocks.io/docs/glossary/accounts/
---

Accounts are the central data structure in AiBlocks. Accounts are identified by a public key and saved in the ledger.
...
>
> #### Balance
> The number of delos held by the account. The balance is denominated in 1/10,000,000th of a delo, the smallest divisible unit of a delo.
>
> #### Sequence number
...
> (optional) Account designated to receive inflation. Every account with a balance of at least 100 DLO can vote to send [inflation](./inflation.md) to a destination account.
>
> #### Flags
...
>
> #### Home domain
> A domain name that can optionally be added to the account. Clients can look up a [aiblocks.toml](./aiblocks-toml.md) from this domain. This should be in the format of a [fully qualified domain name](https://en.wikipedia.org/wiki/Fully_qualified_domain_name) such as `example.com`.
>
> The federation protocol can use the home domain to look up more details about a transaction's memo or [address](https://www.aiblocks.io/developers/learn/concepts/federation.html#aiblocks-addresses) details about an account. For more on federation, see the [federation guide](./federation.md).
>
> #### Thresholds
...
>
> #### Liabilities
> Starting in protocol version 10, each account also tracks its delo liabilities. Buying liabilities equal the total amount of delos offered to buy aggregated over all offers owned by this account, and selling liabilities equal the total amount of delos offered to sell aggregated over all offers owned by this account. An account must always have balance sufficiently above the minimum reserve to satisfy its delo selling liabilities, and a balance sufficiently below the maximum to accomodate its delo buying liabilities.
