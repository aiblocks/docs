...
replacement: https://developers.aiblocks.io/docs/glossary/decentralized-exchange/
---

In addition to supporting the issuing and movement of [assets](./assets.md), the AiBlocks network also acts as a decentralized **distributed exchange** of any type of asset that people have added to the network. Its ledger stores both balances held by user accounts and offers that user accounts make to buy or sell assets.
...
When creating a "buy"/"bid" offer in AiBlocks via the [Manage Buy
...
exchange for 100 DLO, you would specify the price as `{100, 20}`, which would be the equivalent of
1 USD @ 5 DLO.

When creating a "sell"/"offer"/"ask" offer in AiBlocks via the [Manage Sell
...
10 USD in exchange for 100 DLO, you would specify the price as `{100, 10}`, which would be the
equivalent of 1 USD @ 10 DLO (_nice profit_).
...
is because on the AiBlocks network fees are always paid in the native currency of the network
(delos), and are related to the transaction that you submit to the network (which contains your
...
* You place an offer to _buy_ 100 USD for 135 CAD with a fee of 1000
  [stroops](./assets.md#one-stroop-multiple-stroops) (.0001 DLO). You immediately lose .0001
  DLO.
* When your offer is accepted by a seller, you receive 100 USD and exchange 135 CAD in return.
* In total, your gain is 100 USD, and your loss is 135 CAD _and_ .0001 DLO.

For more information, take a look at [our guide on fees in AiBlocks](./fees.md).


## Orderbook
An **orderbook** is a record of outstanding orders on the AiBlocks network. This record sits between any two assets. Abstractly, we often discuss assets using two _fictional placeholder_ assets traded in a market, which we call "**wheat**" or "**sheep**". The orderbook for that asset-pair therefore records every account wanting to sell wheat for sheep on one side _and_ every account wanting to sell sheep for wheat on the other side.

(A bit of further market terminology: outside of AiBlocks, the concept of an "orderbook" normally contains two kinds of "**order**": buying is expressed by "**bid**" orders, and selling is expressed by "**ask**" orders, also called "**offers**". Within the AiBlocks network, the representation of orders is simplified: all orders are stored as **selling** -- i.e. the system automatically converts all bids to asks in the opposite direction -- and AiBlocks's documentation and code can therefore sometimes be a bit lax in using the words "offer" and "order" as synonyms. Since both words refer to the same thing in AiBlocks in all cases, they are occasionally used interchangably. This is harmless in the context of the AiBlocks ecosystem, but outside that context, an "offer" usually means an "ask" order _specifically_!)


An orderbook can be summarized by a diagram, as shown below. It is often visible on the trading interface of an exchange, though sometimes inverted left-to-right or drawn millenniumtally. But the idea is the same.
...
the prices are not equal. For example, if the best offer to buy BTC for DLO has a price of
100DLO/BTC, and you make a passive offer to sell BTC at 100DLO/BTC, your passive offer *does not*
take that existing offer. If you instead make a passive offer to sell BTC at 99DLO/BTC it would
cross the existing offer and fill at 100DLO/BTC.
...
Suppose you are holding sheep and want to buy something from a store that only accepts wheat. You can create a payment in AiBlocks that will automatically convert your sheep into wheat. It goes through the sheep/wheat orderbook and converts your sheep at the best available rate.
...
This process of finding the best path of a payment is called **pathfinding**. Pathfinding involves looking at the current orderbooks and finding which series of conversions gives you the best rate. It is handled outside of AiBlocks Core by something like Millennium.


## Preferred currency
Because cross-asset payments are so simple with AiBlocks, users can keep their money in whatever asset they prefer to hold. **Preferred currency** creates a very flexible, open system.
